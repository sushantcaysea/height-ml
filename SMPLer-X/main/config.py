...
class Config:
    ...
    def prepare_dirs(self, exp_name):
        ...
        ## copy some code to log dir as a backup (Windows-safe)
        ...
        for file in copy_files:
            src = osp.join(self.root_dir, file.replace('/', os.sep))
            if not osp.exists(src):
                continue
            dst = osp.join(self.code_dir, osp.basename(src))
            if osp.isdir(src):
                if osp.exists(dst):
                    shutil.rmtree(dst)
    ...
    def update_config(self, num_gpus, exp_name):
        ...
        # Save config backup (skip yapf dump — breaks on newer yapf/Windows)
        try:
            cfg_save = MMConfig(self.__dict__)
            with open(osp.join(self.code_dir, 'config_base.py'), 'w', encoding='utf-8') as f:
                f.write(str(cfg_save._cfg_dict))
        except Exception:
            pass
