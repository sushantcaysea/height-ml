...
# Windows has no EGL; use win32. Lazy-import pyrender only if needed.
...
def render_mesh(img, mesh, face, cam_param, mesh_as_vertices=False):
    # Prefer vertex overlay on Windows / when OpenGL EGL is unavailable
    if mesh_as_vertices or sys.platform.startswith('win'):
        vertices_2d = perspective_projection(mesh.copy(), cam_param)
